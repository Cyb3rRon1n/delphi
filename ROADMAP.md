...
1. **Does Firefox even start the background context from `background.service_worker` alone?** Unconfirmed; `strict_min_version: "121.0"` is pinned because pre-121 Firefox had a real bug here. Load the unpacked extension via `about:debugging` on current Firefox and exercise one path end-to-end. If the worker doesn't run, the honest fix is a small two-manifest packaging step (see the rejected-approaches note in CLAUDE.md — a single manifest carrying both keys is confirmed dead), not another single-manifest guess.
2. **Side panel on Firefox.** Check current MDN + Firefox release notes for whether Firefox has adopted the standardized `side_panel` key (the earlier revert happened precisely because this was assumed instead of checked). If unsupported: two manifests become the plan-of-record for both gaps at once, and Firefox keeps the Options fallback meanwhile.
